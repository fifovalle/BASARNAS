...
<body>
	...
	<section class="table-samapta">
		...
		<div class="d-flex align-items-center">
			...
			<button class="btn btn-round ms-auto tombol-tambah" data-bs-toggle="modal" data-bs-target="#tambahGarjasShuttleRun">
			...
			<table class="table">
				...
				<tbody class="table-group-divider text-center" id="shuttlerunTabelBody">
					...
					if ($jenisKelamin == 'Pria') {
						...
						$statusField = 'Status_Pria_Shuttle_Run';
						...
						$statusField = 'Status_Wanita_Shuttle_Run';
					} else {
						$shuttleRunInfo = null;
					}
					...
					if (!empty($shuttleRunInfo)) {
						foreach ($shuttleRunInfo as $shuttleRun) {
							if (isset($shuttleRun[$statusField])) {
								if ($shuttleRun[$statusField] == 'Ditolak') {
								...
								} elseif ($shuttleRun[$statusField] == 'Ditinjau') {
	...
	<script>
		$(document).ready(function() {
			$('.dropdown-item').on('click', function() {
				...
				if (jumlahBaris == 0) {
					$('#shuttlerunTabelBody').append("<tr id='barisTidakAdaData'><td colspan='4' class='text-center text-danger fw-bold'>Tidak ada data Shuttle Run.</td></tr>");
				}
			});
	...
	<!-- MODALS START -->
	<?php include('../partials/modal-add-garjas-shuttlerun.php'); ?>
	<!-- MODALS END -->
</body>
